...
# STEP 2 propagated residual -- NON-absorber only. Solve a card's lone unknown ONLY when it is NOT an
# absorber sig (CXC or citer); a still-unknown absorber keeps the card "unresolved" here, deferring it so
# it becomes the residual ABSORBER later (the hardest part to measure directly: gated + pay-to-assemble).
...
# STEP 3 estimated (family median). 3a: estimate every NON-absorber sig first, so the lone unknown left on
# an absorber card is its CXC / citer sig. 3b: derive each absorber sig as the residual absorber (delta -
# sum(others)), floored. 3c: estimate any leftover (CXC -> floored CXC median, else family median).
...
for sig in ALLV:                                   # 3a non-absorber family estimate
...
for _ in range(10):                                # 3b absorber residual absorber (CXC + citers)
    res = collections.defaultdict(list)
    for cn, dl, sg, e in multi:
        unk = [s for s in sg if s not in cost]
    ...
    for sig, samples in res.items():
        if sig in cost: continue
        v = mode500(samples)
        # CXC floored at >= 500; a citer floored at >= 0 (the folded replay body never gives power back)
