...
#
# The per-ability cost MATH lives in cost_model.py (the single source, shared with build_db.py). This
# file owns only the Excel I/O around it: openpyxl sheet generation, example-card aggregation, help text.
import json, os, re, collections, glob
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment
import official_en
from cost_model import _nk, pb, ra, base_num, gen, family, ability_type, build_cost_model, CONF
...
era = json.load(open(os.path.join(D, "card_era.json"), encoding="utf-8"))   # FORMAT names; metadata only (DBG)
try:
...
# The full per-ability cost cascade (replay folding + measured -> residual -> estimated, Characters only)
# lives in cost_model.py and is shared verbatim with build_db.py. Run it over the de-duplicated cards and
# read the results off the returned object; this file only does the Excel I/O around it.
...
if M.validation_pct is not None:
    print(f"VALIDATION: |err|<=500 in {M.validation_pct:.0f}% (n={M.validation_n})")
# local aliases so the Excel-generation + DBG code below reads the model results directly
variant_occ = M.variant_occ; variant_text = M.variant_text
cost = M.cost; method = M.method; nsamp = M.nsamp; rng = M.rng
ALLV = M.ALLV; cards = M.char_cards   # cards = [(card_number, delta, [sig...], era)] -- era is None now
...
DPRIN = [
 ...
 ("• No era multiplier: a package's cost is a FIXED standard (the MODE of its samples across ALL years). No power-creep at the package level — date is metadata, not a cost driver.", False),
