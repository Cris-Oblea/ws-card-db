...
| `card_era.json` | `card_number → era` (legacy/modern by default). Generated by `ingest/build_card_era.py` from `set_dates.json`; edit its `ERA_BUCKETS` to add finer eras (e.g. standby/choice). |
